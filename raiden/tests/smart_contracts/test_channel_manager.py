...
# def test_deleteChannel(tester_state, tester_token, tester_events,
    ...
    # assert tester_token.approve(netting_contract_proxy1.address, 100, sender=key0) is True
    # assert tester_token.approve(netting_contract_proxy1.address, 100, sender=key1) is True
    # assert netting_contract_proxy1.deposit(100, sender=key0) is True
    # assert netting_contract_proxy1.deposit(100, sender=key1) is True
    # assert tester_state.block.number > 0
    # assert netting_contract_proxy1.opened(sender=key0) > 0

    # assert channel.distributable > 0
...
def test_deleteChannel(tester_state, tester_channelmanager, tester_channels, settle_timeout):
    ...
    previous_count = len(tester_channelmanager.getChannelsParticipants(sender=privatekey0_raw))
    tester_channelmanager.deleteChannel(
        address0,
        nettingchannel.address,
        sender=privatekey0_raw,
    )
    assert previous_count - 2 == len(tester_channelmanager.getChannelsParticipants(sender=privatekey0_raw))
