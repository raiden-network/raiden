...
def test_service_registry_random_pfs(
    ...
    assert c1_service_proxy.ever_made_deposits_len(BLOCK_SPEC_LATEST) == 3

    # Test that getting the url for each service address works
    for idx, address in enumerate(addresses):
        assert c1_service_proxy.get_service_url(BLOCK_SPEC_LATEST, address) == urls[idx]
    # Test that getting the url for a non-existing service address returns None
    assert c1_service_proxy.get_service_url(BLOCK_SPEC_LATEST, HOP1) is None

    # Test that get_service_address by index works
    for idx, address in enumerate(addresses):
        assert c1_service_proxy.ever_made_deposits(BLOCK_SPEC_LATEST, idx) == address

    # Test that getting the address for an index out of bounds returns None
    assert not c1_service_proxy.ever_made_deposits(BLOCK_SPEC_LATEST, 9999)
    ...
    with patch("raiden.network.pathfinding.get_pfs_info", mock_get_pfs_info):
        ...
        assert not get_valid_pfs_url(
            c1_service_proxy, 0, BLOCK_SPEC_LATEST, pathfinding_max_fee=FeeAmount(99)
        )

        # ...but ones with the expected price are fine
        assert (
            get_valid_pfs_url(
                c1_service_proxy, 0, BLOCK_SPEC_LATEST, pathfinding_max_fee=FeeAmount(100)
            )
            == urls[0]
        )
        ...
        assert (
            get_random_pfs(c1_service_proxy, BLOCK_SPEC_LATEST, pathfinding_max_fee=FeeAmount(100))
            in urls
        )
