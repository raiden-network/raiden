...
def test_fullnetwork(
    ...
    app0_state_changes = [
        change[1] for change in get_all_state_changes(app0.transaction_log)
        if not isinstance(change[1], Block)
    ]
    app0_events = [
        event.event_object for event in get_all_state_events(app0.transaction_log)
    ]
    app1_state_changes = [
        change[1] for change in get_all_state_changes(app1.transaction_log)
        if not isinstance(change[1], Block)
    ]
    app1_events = [
        event.event_object for event in get_all_state_events(app1.transaction_log)
    ]
    app2_state_changes = [
        change[1] for change in get_all_state_changes(app2.transaction_log)
        if not isinstance(change[1], Block)
    ]
    app2_events = [
        event.event_object for event in get_all_state_events(app2.transaction_log)
    ]
    app3_state_changes = [
        change[1] for change in get_all_state_changes(app3.transaction_log)
        if not isinstance(change[1], Block)
    ]
    app3_events = [
        event.event_object for event in get_all_state_events(app3.transaction_log)
    ]
    ...
    assert app0_state_changes[1].our_address == app0.address
    assert app0_state_changes[1].transfer.amount == amount
    assert app0_state_changes[1].transfer.token == token_address
    assert app0_state_changes[1].transfer.initiator == app0.address
    assert app0_state_changes[1].transfer.target == app2.address
    ...
    assert app0_state_changes[2].sender == app2.address
    ...
    assert app0_state_changes[3].sender == app3.address
    ...
    assert app0_events[1].initiator == app0.address
    assert app0_events[1].target == app2.address
    assert app0_events[1].receiver == app3.address
    ...
    assert app0_events[2].receiver == app2.address
    assert app0_events[2].sender == app0.address
    ...
    assert app0_events[3].receiver == app3.address
    ...
    assert app3_state_changes[0].our_address == app3.address
    # We should have only 1 available route from mediator to target
    from_route = RouteState(
        state='opened',
        node_address=app0.address,
        channel_address=channel_0_3.channel_address,
        available_balance=deposit,
        settle_timeout=settle_timeout,
        reveal_timeout=reveal_timeout,
    ...
    assert app3_state_changes[0].from_transfer.initiator == app0.address
    assert app3_state_changes[0].from_transfer.target == app2.address
    ...
    assert app3_state_changes[1].sender == app2.address
    ...
    for state_change in app3_state_changes[2:]:
        ...
        assert state_change.sender == app0.address
    ...
    assert app3_events[0].initiator == app0.address
    assert app3_events[0].target == app2.address
    assert app3_events[0].receiver == app2.address
    ...
    assert app3_events[1].receiver == app0.address
    assert app3_events[1].sender == app3.address
    ...
    assert app3_events[2].receiver == app2.address
    ...
    assert app2_state_changes[0].our_address == app2.address
    ...
    assert app2_state_changes[0].from_transfer.initiator == app0.address
    assert app2_state_changes[0].from_transfer.target == app2.address

    # We also get secret reveals from the initiator and the mediator.
    assert isinstance(app2_state_changes[1], ReceiveSecretReveal)
    assert app2_state_changes[1].sender == app0.address
    ...
    assert app2_events[0].receiver == app0.address
    ...
    assert app2_events[1].receiver == app3.address
    assert app2_events[1].sender == app2.address
