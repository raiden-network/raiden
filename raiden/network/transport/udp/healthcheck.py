...
def healthcheck(
    ...
    log.debug("starting healthcheck for", node=pex(transport.address), to=pex(recipient))
    ...
    try:
        ...
        log.debug(
            "waiting for endpoint registration", node=pex(transport.address), to=pex(recipient)
        )
        ...
        backoff = udp_utils.timeout_exponential_backoff(
            nat_keepalive_retries, nat_keepalive_timeout, nat_invitation_timeout
        )
        sleep = next(backoff)
