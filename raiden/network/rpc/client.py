...
class TransactionGuard:
    def __init__(self, client: "JSONRPCClient", available_nonce: Nonce):
        self._client = client
    ...
    @property
    def available_nonce(self) -> Nonce:
        return self._available_nonce
...
class JSONRPCClient:
    ...
    def __init__(
        ...
        self.transaction_guard = TransactionGuard(self, available_nonce)
    ...
    def __repr__(self) -> str:
        return (
            f"<JSONRPCClient "
            f"node:{to_checksum_address(self.address)} "
            f"nonce:{self.transaction_guard.available_nonce}"
            f">"
        )
    ...
    def deploy_single_contract(
        ...
        with self.transaction_guard as guard:
            transaction_hash = guard.send_transaction(
                to=Address(b""),
