...
class PortMappedSocket:
    ...
    def __repr__(self):
        return "PortMappedSocket<{} mapping: {}:{} ({})>".format(
            self.socket.__repr__(), self.external_ip, self.external_port, self.method
        )
...
class SocketFactory:
    ...
    def __enter__(self):
        ...
        if method == "ext":
            method = "ext:{}".format(":".join(str(s) for s in self.strategy_args))
        log.info(
            "Network port opened",
            method=method,
    ...
    def map_upnp(self):
        ...
        try:
            result = upnpsock.open_port(router, self.source_port)
    ...
    def map_stun(self):
        try:
            external_ip, external_port, nat = stunsock.stun_socket(
                self.socket, self.source_ip, self.source_port, **self.kwargs
            )
    ...
    def map_none(self):
        if self.source_ip == "0.0.0.0":
            try:
                ...
                log.critical(
                    "Couldn't get interface address. " "Try specifying with '--nat ext:<ip>'."
                )
                raise
    ...
    def _open_socket(self):
        ...
        try:
            ...
            log.debug("Socket opened", ip=sock.getsockname()[0], port=sock.getsockname()[1])
            self.socket = sock
        except socket.error as e:
            if e.errno == errno.EADDRINUSE:
                raise RaidenServicePortInUseError()
