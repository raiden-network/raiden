...
.. attention:: Parity sometimes loses its historical DB (potentially after updates). Due to this some events might be lost which will result in Raiden not being able to fetch all event. Therefore it is recommended to make sure to have Parity fully synced with `--no-warp`.
