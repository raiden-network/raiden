...
In Raiden we distinguish between two types of events: blockchain_events and raiden_events. blockchain_events are events that are being emitted by the smart contracts, as for raiden_events, those are events happening in the Raiden node for now, it's being used for debugging purpose. Be aware: All raiden events endpoints might get removed in the future. 
...
.. http:get:: /api/(version)/blockchain_events/tokens/(token_address)
   ...
   **Example Response**:
   ...
   :statuscode 200: For successful query
   :statuscode 400: If the provided query string is malformed
   :statuscode 404: If the token_address does not exist
...
.. http:get:: /api/(version)/blockchain_events/payment_networks/(token_address)/channels/(partner_address)
   ...
   If the partner_address is not provided it will show the evnts for all the channels.
...
.. http:get:: /api/(version)/raiden_events/tokens/(token_address)
  ...
  **Example Response**:
  ...
  .. sourcecode:: http
    ...
    [
        {
            block_number: 3739741,
            event: "SendProcessed"
        },
        {
            block_number: 3694271,
            event: "ContractSendChannelClose"
        },
        {
  ...
  .. sourcecode:: http
    ...
    [
        ...
        },
        {
            ...
            recipient: "0x82641569b2062b545431cf6d7f0a418582865ba7",
            queue_name: "0xa152038763d73b05df7b036f477236b527ad14a249e4077fb4048d845226ac43",
            message_identifier: 14913626383601089000,
            transfer: "<LockedTransferUnsignedState id:1532341917127 token:0x0f114a1e9db192502e7856309cc899952b3db1ed balance_proof:<BalanceProofUnsignedState nonce:3 transferred_amount:20 locked_amount:35 locksroot:dc39c0b3 token_network:7692624c channel_address:a1520387 chain_id: 3> lock:<HashTimeLockState amount:35 expiration:3694845 secrethash:02db535c> target:0x82641569b2062b545431cf6d7f0a418582865ba7>"
        }, ...
    ]
    
    :statuscode 200: For successful query
    :statuscode 400: If the provided query string is malformed
    :statuscode 404: If the provided token_address does not exist
