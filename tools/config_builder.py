...
@click.argument("nodes_per_host", default=1, type=int)
@cli.command()
@click.pass_context
...
@click.argument("hosts", nargs=-1, type=str)
@click.argument("nodes_per_host", default=1, type=int)
@click.argument("nodes_per_transfer", default=2, type=int)
@cli.command()
@click.pass_context
def build_scenario(ctx, hosts, nodes_per_host, nodes_per_transfer):
    ...
    for token_num in range(total_tokens):
        data_for_token = {
            ...
            "transfers_with_amount": {addresses[index + nodes_per_transfer - 1]: 3000},
        }
...
@click.argument("geth_hosts", nargs=-1, type=str)
...
@click.argument("genesis_json", type=click.File())
@click.argument("state_json", type=click.File())
@cli.command()
@click.pass_context
...
def full_genesis(ctx, hosts, nodes_per_host, scenario):
    ...
    if scenario is not None:
        ...
        token_groups = {token["name"]: token["channels"] for token in script["tokens"]}
    else:
        # create tokens for addresses x addresses
        token_groups = {account["address"]: all_addresses for account in accounts.values()}
