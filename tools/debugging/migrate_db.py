...
def upgrade_db(current_version: int, new_version: int):
    ...
    manager = UpgradeManager(
        db_filename=database_path, current_version=current_version, new_version=new_version
    )
    try:
        manager.run()
...
def main(db_file):
    ...
    migrate_db(storage=sqlite.SerializedSQLiteStorage(db_file, serialize.JSONSerializer()))
