...
token2='${TOKEN2}'
token3='${TOKEN3}'
am1=raiden.default_registry.token_network_by_token('${TOKEN1}'.decode('hex'))
am2=raiden.default_registry.token_network_by_token('${TOKEN2}'.decode('hex'))
# am3=raiden.default_registry.token_network_by_token('${TOKEN3}'.decode('hex'))"
...
[ "$SETUP_CHANNELS" -eq 1 ] && {
    ...
    tmux send-keys -t raiden:3 "tools.token_network_register(raiden.default_registry.address, '${TOKEN1}')" C-m
    tmux send-keys -t raiden:3 "tools.token_network_register(raiden.default_registry.address, '${TOKEN2}')" C-m
    # tmux send-keys -t raiden:3 "tools.token_network_register(raiden.default_registry.address, '${TOKEN3}')" C-m
