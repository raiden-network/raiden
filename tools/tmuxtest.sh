...
[ "$SETUP_VARIABLES" -eq 1 ] && {
    ...
    tmux send-keys -t raiden:3 "raiden.api.expect_exchange('${TOKEN1}'.decode('hex'), 100, '${TOKEN2}'.decode('hex'), 70,  '${ADDRESS3}'.decode('hex'))"
    tmux send-keys -t raiden:5 "raiden.api.exchange('${TOKEN1}'.decode('hex'), 100, '${TOKEN2}'.decode('hex'), 70,  '${ADDRESS1}'.decode('hex'))"
}
